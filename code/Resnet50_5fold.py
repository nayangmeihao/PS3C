...
from modules import CustomDataset_v1, calculate_mean_std, EarlyStopping, get_logger
...
def train(dataset, batch_size, num_epochs=30, nclass=3):
    ...
    data_indices = list(range(len(dataset)))  # 获取数据集索引
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(data_indices)):
        ...
        # ResNet 的结构包括多个子模块，我们需要找到最后 10 层
        all_layers = list(model.children())  # 提取所有子模块
        logger.info(f"Total layers: {len(all_layers)}")  # 打印总层数

        # 根据 ResNet 层级结构，保留最后几个模块
        ...
        logger.info("开始执行epoch...")
        time_start = int(time.time() * 1000)
        for epoch in range(num_epochs):
            model.train()
            running_loss = 0.0
            all_preds_train, all_labels_train = [], []
            logger.info("epoch:{}开始加载和处理batch训练数据...".format(epoch))
            for inputs, labels in train_loader:
                # logger.info("开始加载和处理batch训练数据...")
                inputs, labels = inputs.to(device), labels.to(device)
                optimizer.zero_grad()
                # logger.info("获取前向计算结果")
            ...
            logger.info("epoch:{}处理batch完成".format(epoch))
            ...
            logger.info("epoch{0}耗时：{1}毫秒".format(epoch, time_cost))
            ...
            if epoch >= 40:
                ...
                if f1_test > best_f1:
                    ...
                    torch.save(model, best_model_path)  # 保存整个模型（包括结构和参数）
